...
push @SPECHOOKS, sub {
    ...
    $jpp->get_section('install')->unshift_body2_after(q!%{__sed} -i 's/project name=""/project name="%{name}"/;s/id="commons-collections"/id="apache-collections"/' %{buildroot}%{repodir}/component-info.xml
