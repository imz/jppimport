...
push @SPECHOOKS, 
 sub {
    my ($spec, $parent) = @_;
    &add_missingok_config($spec, '/etc/java/aspectj.conf','');
    #$spec->add_patch('aspectj-ant_0_8_fix.diff',STRIP=>2);
    $spec->get_section('package','')->subst_if('saxon(\s+[>=]+\s+[\d:\-\.]+)?','saxon6',qr'Requires:');
    $spec->get_section('prep')->subst(qr'build-classpath saxon','build-classpath saxon6');

    # for 1.5.4. Deprecated?
    $spec->get_section('package','')->unshift_body('BuildRequires: ant-apache-regexp ant-apache-xalan2'."\n");
    # ant 1.8
    $spec->get_section('prep')->push_body(q!sed -i -e 's,<antcall,<antcall inheritRefs="true",g'  build/build-properties.xml!."\n");
    $spec->get_section('build')->subst(qr'-Xmx512','-Xmx1024');
    $spec->get_section('install')->subst(qr'\%{buildroot}\%{_libdir}/eclipse','%{buildroot}%{_datadir}/eclipse');
    $spec->get_section('files','eclipse-plugins')->subst(qr'\%{_libdir}/eclipse','%{_datadir}/eclipse');
}
__END__
#    $spec->get_section('package')->subst('saxon-scripts','saxon6-scripts');
#    $spec->applied_block(
#    	"bin saxon6 hook",
#	sub {
#    $spec->get_section('build')->subst('/usr/bin/saxon','/usr/bin/saxon6');
#    $spec->get_section('prep')->subst('/usr/bin/saxon','/usr/bin/saxon6');
#    $spec->get_section('build')->subst('saxon\s+-o','saxon6 -o');
#    $spec->get_section('prep')->subst('saxon\s+-o','saxon6 -o');
#    });
